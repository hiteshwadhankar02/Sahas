...
export default function Reports({dbpath}) {
    ...
    const onSearch = () => {
      ...
      if(type1==='1')
      {
        ...
        if(filter1==='1')
        {
          query='select * from rregister where validtill < "'+todayDate+'" AND validtill LIKE "%-%" AND id != 0;';
          nextProcess(query);
        }
        else if(filter1==='2')
        {
          query='select * from rregister where validtill < "'+nextWeekDate+'" And validtill > "'+todayDate+'" AND validtill LIKE "%-%" AND id != 0;';
          nextProcess(query);
        }
        else if(filter1==='3')
        {
          query='select * from rregister where validtill NOT LIKE "%-%" AND id != 0;';
          nextProcess(query);
        }
        else
        {
      ...
      else if(type1==='2')
      {
        ...
        if(filter1==='1')
        {
          query='select * from tregister where validtill < "'+todayDate+'" AND validtill LIKE "%-%" AND id != 0;';
          nextProcess(query);
        }
        else if(filter1==='2')
        {
          query='select * from tregister where validtill < "'+nextWeekDate+'" And validtill > "'+todayDate+'" AND validtill LIKE "%-%" AND id != 0;';
          nextProcess(query);
        }
        else if(filter1==='3')
        {
          query='select * from tregister where validtill NOT LIKE "%-%" AND id != 0;';
          nextProcess(query);
        }
        else
        {
          ...
          <div className="mb-3" style={{marginLeft:'32%', display:'flex'}}>
            ...
            <div className="input-group mb-3" >
              <select className="form-select" id="inputGroupSelect01"  value={type1} onChange={(e) => setType1(e.target.value)} >
                ...
                <option value="1">Recreatioal</option>
